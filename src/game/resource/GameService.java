...
public class GameService {
	...
	public static final int SIZE = 100;// 次背景方格元素间距
	long starttime;
	...
	/**
	 * 画出游戏面板上的内容：1.分数板图片以及分数数值  2，次背景方格内的数字图片
	 * @param g    画笔
	...
	//上消除，键盘上方向键触发，下边的图片尝试上边合并 因为在上消除之前已经调用了上移动，所以，格子都已经上移到上边
	public int removeUp(){
		int isRemove=0;
		for(int i=0;i<3;i++){
			for(int j=0;j<4;j++){
	...
	//下消除，键盘下方向键触发，上边的图片尝试下边合并 因为在下消除之前已经调用了下移动，所以，格子都已经下移到下边
